...
import { ExtensionApi, GITHUB_LINK, REVIEW_LINK, SPONSOR_LINK } from '@frontmatter/common';
...
export const WelcomeScreen: React.FunctionComponent<IWelcomeScreenProps> = ({}: React.PropsWithChildren<IWelcomeScreenProps>) => {
  const settings = useRecoilValue(SettingsSelector);
  const { get } = useExtension();

  useEffect(() => {
    return () => {
      get(ExtensionApi.reload);
    };
  }, ['']);
