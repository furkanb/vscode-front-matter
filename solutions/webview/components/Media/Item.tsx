...
import { MediaInfo, ScriptType, parseWinPath, MediaApi, ExtensionApi, CustomScript } from '@frontmatter/common';
...
export const Item: React.FunctionComponent<IItemProps> = ({media}: React.PropsWithChildren<IItemProps>) => {
  ...
  const copyToClipboard = () => {
    ...
    post(ExtensionApi.copyToClipboard, { data: parseWinPath(relPath) || "" });
  };

  const runCustomScript = (script: CustomScript) => {
    post(MediaApi.runCustomScript, { script, path: media.fsPath });
  };

  const insertToArticle = () => {
    const relPath = getRelPath();
    post(MediaApi.insertPreviewImage, {
