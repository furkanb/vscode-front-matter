...
import { Status, Framework, FrameworkDetectors, Settings, ExtensionApi } from '@frontmatter/common';
...
export const StepsToGetStarted: React.FunctionComponent<IStepsToGetStartedProps> = ({settings}: React.PropsWithChildren<IStepsToGetStartedProps>) => {
  ...
  const setFrameworkAndSendMessage = (framework: string) => {
    ...
    post(SettingsApi.framework, { framework });
  }

  const initializeProject = async () => {
    await get(ExtensionApi.init);
    await fetchSettings();
  };

  const reload = async () => {
    await get(ExtensionApi.reload);
  };
  ...
  const steps = [
    { 
      ...
      onClick: settings.initialized ? undefined : initializeProject 
    },
    { 
    ...
    },
    {
      ...
      onClick: (settings.initialized && settings.folders && settings.folders.length > 0) ? reload : undefined
    }
  ];
